...
keystrokes on its own. Letters follow your keyboard layout (on AZERTY, `Ctrl+Z`
is still undo). On layouts without Latin letters, such as Cyrillic or Greek,
they fall back to the key's position, and tab numbers always use the number row.
`Ctrl+Shift+P` and `Ctrl+Shift+G` are left to Excalidraw (command palette,
ungroup).
...
and **Kanagawa Lotus**. *Follow system* tracks the desktop's light/dark preference and
picks from a pair (Kanagawa Lotus / Kanagawa Wave by default). The preference
is read from the XDG desktop portal, so it works on GNOME, KDE and any other
desktop that ships one, and the app switches the moment it changes. Without a
portal it falls back to GNOME's `gsettings`, checked whenever the window gains
focus.
...
The **Export** menu remembers three choices, stored in
`~/.config/excalidraw-desktop/export.json`:
...
Exports are suggested next to the drawing they came from.

## Library

The Library sidebar is saved to `~/.config/excalidraw-desktop/library.excalidrawlib`
and comes back on the next launch. Excalidraw's component keeps it in memory
only, so this app stores it itself.

A first launch starts the library with a set of **entity–relationship diagram
shapes** (entities, relationships, attributes, and crow's-foot connectors).
Remove the ones you do not want; **View → Add ER diagram shapes to library**
adds any that are missing back. They are generated by
`scripts/build-er-library.mjs` into `resources/libraries/`.
...
src/                    Renderer — React 19 + Excalidraw. No filesystem access.
  ...
  lib/library.ts        Saves the Library sidebar; seeds and re-adds the ER shapes
  lib/exportPreferences.ts  The Export menu's remembered choices
  lib/shortcuts.ts      Which key a Ctrl shortcut means, across keyboard layouts
...
src-tauri/src/          Rust backend
  ...
  library.rs            Reads and writes the saved Library
...
- `themes/`
- `session/`, the autosave snapshots, readable only by you
